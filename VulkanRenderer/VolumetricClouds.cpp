...
bool VolumetricClouds::Init(VKRenderer* renderer)
{
	...
	colorParams.usedInCopySrc = true;
	colorParams.usedInCopyDst = false;
	...
	renderer->UpdateUserTextureSet2D(cloudReprojectionSet[0], previousFrameTexture[0], 1);
...
void VolumetricClouds::PerformCloudsPass(VKRenderer *renderer, VkCommandBuffer cmdBuffer)
{
	...
	viewport.width = cloudsLowResFB.GetWidth();
	viewport.height = cloudsLowResFB.GetHeight();
...
void VolumetricClouds::PerformCloudsReprojectionPass(VKRenderer* renderer, VkCommandBuffer cmdBuffer)
{
	...
	VkViewport viewport = {};
	viewport.maxDepth = 1.0f;
	viewport.width = cloudsReprojectionFB.GetWidth();
	viewport.height = cloudsReprojectionFB.GetHeight();
	...
	// We don't need to bind the vb again, this pass is performed right after the Cloud Pass
	//vertexBuffers[0] = quadMesh.vb.GetBuffer();
	//vkCmdBindVertexBuffers(cmdBuffer, 0, 1, vertexBuffers, offsets);
	vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, cloudReprojectionMat.GetPipeline());
	vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->GetPipelineLayout(), 2, 1, &cloudReprojectionSet[(renderer->GetCurrentFrame() + 1) % 2], 0, nullptr);
	vkCmdDraw(cmdBuffer, 6, 1, 0, 0);
	...
	// Transition the images to be read for the blit
	base.TransitionImageLayoutCmdBuffer(cmdBuffer, srcImg, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
	base.TransitionImageLayoutCmdBuffer(cmdBuffer, dstImg, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
